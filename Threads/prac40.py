...
with concurrent.futures.ThreadPoolExecutor() as executor:
    seconds = [5, 4, 3, 2, 1]
    a = [executor.submit(f1, sec) for sec in seconds] # List Comprehension

    # "as_completed" method gives the result.
    for f in concurrent.futures.as_completed(a):
        print(f.result())          # gives the return value in the
